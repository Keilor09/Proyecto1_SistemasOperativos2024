...
// Función para obtener el porcentaje total de utilización del disco en GiB
...
void obtenerLibreDiscoGiB(int pipe_fd[]) {
    // Cerrar el extremo de lectura del pipe en el proceso hijo
    close(pipe_fd[0]);
    ...
    // Escribir las estadísticas en el pipe
    char output[1024]; // Buffer para almacenar el mensaje de salida
    sprintf(output, "(GiB) Porcentaje libre del disco: %f%%\n", porcentajeLibre);
    write(pipe_fd[1], output, strlen(output));
...
void obtenerLibreDiscoMiB(int pipe_fd[]) {
    // Cerrar el extremo de lectura del pipe en el proceso hijo
    close(pipe_fd[0]);
    ...
    // Escribir las estadísticas en el pipe
    char output[1024]; // Buffer para almacenar el mensaje de salida
    sprintf(output, "(MiB) Porcentaje libre del disco: %f%%\n", porcentajeLibre);
    write(pipe_fd[1], output, strlen(output));
